(() => {
  ...
  function loadStyle(href) {
    ...
    const link=document.createElement("link"); link.rel="stylesheet"; link.href=`${href}?v=8`; document.head.appendChild(link);
  }
  function loadScript(src) {
    if(document.querySelector(`script[src^="${src}"]`)) return;
    const script=document.createElement("script"); script.src=`${src}?v=8`; script.defer=true; document.head.appendChild(script);
  }
  function bootAddons(){
